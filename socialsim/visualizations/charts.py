...
def histogram(df, xlabel, ylabel, title, **kwargs):
    ...
    if 'Ground Truth' in df.columns:
        ax.hist(gold_data, bins, label='Ground Truth', alpha=0.7, color='green')
    if 'Simulation' in df.columns:
        ax.hist(test_data, bins, label='Simulation', alpha=.7, color='red')
...
def multi_histogram(df, xlabel, ylabel, title, **kwargs):
    n_bins = 100

    cols = [c for c in df.columns if c not in ['Ground Truth', 'Simulation']]

    unique = df.drop_duplicates(subset=cols)

    row = 1 + int( (len(unique) - 1) /3)
    col = min(3,len(unique))
    ...
    for i in range(len(unique)):
        ...
        data = df.copy()
        for c in cols:
            data = data[data[c] == unique.iloc[i][c]]
        ...
        if 'Simulation' in data.columns and 'Ground Truth' in data.columns:

            gold_data = data.dropna(subset=["Ground Truth"])["Ground Truth"]
            test_data = data.dropna(subset=["Simulation"])["Simulation"]
       
            data = np.concatenate([gold_data, test_data])
    
        elif 'Simulation' in data.columns or 'Ground Truth' in data.columns:

            if 'Simulation' in data.columns:
                data2 = data.dropna(subset=["Simulation"])['Simulation']
                test_data = data2.copy()
            else: 
                data2 = data.dropna(subset=["Ground Truth"])['Ground Truth']
                gold_data = data2.copy()
        else:
            return None

        _,bins = np.histogram(data2,bins='doane')
        ...
        ax.set(xlabel=xlabel)
        ax.set(ylabel=ylabel)
        ax.set(title=' - '.join(unique.iloc[i][cols].values))
        ax.legend(loc='best')
...
def heatmap(df, xlabel, ylabel, title, **kwargs):

    cols = [c for c in df.columns if c not in ['Ground Truth','Simulation'] ]
    values = [c for c in df.columns if c in ['Ground Truth', 'Simulation'] ]

    fig, axes = plt.subplots(1, len(values), figsize=(7.5*len(values), 5))
    ...
    if len(cols) == 2:
        for i, _ in enumerate(values): 
            ...
            data = df[cols + [values[i]]]
            data = pd.pivot_table(data,index=cols[0],columns=cols[1],values=values[i]).fillna(1.0)
...
def time_series(df, xlabel, ylabel, title, **kwargs):
    ...
    df = df.melt(id_vars = [c for c in df.columns if c not in ['Ground Truth','Simulation']], var_name='type', value_name='vals').sort_values(['type',df.columns[0]])

    df.dropna(inplace=True)
    sns.lineplot(x=df.columns[0], y='vals', hue='type', data=df, ax=ax, palette=palette, alpha=0.7)
