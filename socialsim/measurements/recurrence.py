...
class RecurrenceMeasurements(MeasurementsBaseClass):
    ...
    def time_between_bursts(self):
        ...
        '''
#         print([(start_j - end_i).total_seconds() / (60*60*24) for (_, end_i), (start_j, _) in zip(self.burst_intervals[:-1], self.burst_intervals[1:])])
        return np.mean([(start_j - end_i).total_seconds() / (60*60*24) for (_, end_i), (start_j, _) in zip(self.burst_intervals[:-1], self.burst_intervals[1:])])
...
#         print(num_new_users)
    ...
    def lifetime_of_each_burst(self):
        '''
        How many minutes does each burst last on average?
        '''
...
class RecurrenceCommunityMeasurements(MeasurementsBaseClass):
    def __init__(self, dataset_df, id_col='id_h', timestamp_col="nodeTime", userid_col="nodeUserID", platform_col="platform", configuration={}, meatadata=None, communities=None, community_col="coin", log_file='recurrence_measurements_log.txt', node_list=None, community_list=None):
        """
        :param dataset_df: dataframe containing all posts for all coins in all platforms
        :param timestamp_col: name of the column containing the time of the post
        :param id_col: name of the column containing the post id
        :param userid_col: name of the column containing the user id
        """
        ...
        self.platform_col       = platform_col
        self.measurement_type   = 'recurrence'
        self.get_percoin_recurrence_measurements()

    def get_percoin_recurrence_measurements(self):
        self.percoin_recurrence_measurements = {}
        for coin, coin_df in self.dataset_df.groupby(self.community_col):
            self.percoin_recurrence_measurements[coin] = RecurrenceMeasurements(dataset_df=self.dataset_df, id_col=self.id_col, timestamp_col=self.timestamp_col, userid_col=self.userid_col, platform_col=self.platform_col, configuration=self.configuration, community_col=self.community_col)

    def run_for_all_coins(self, measurement_name):
        return [getattr(percoin_recurrence_measurements, measurement_name)() for coin, percoin_recurrence_measurements in self.percoin_recurrence_measurements.items()]
        # for coin, percoin_recurrence_measurements in self.percoin_recurrence_measurements.items():
        #     to_run = getattr(percoin_recurrence_measurements, measurement_name)()
        #     print(to_run)
    ...
    def distribution_of_number_of_bursts(self):
        ...
        '''
        return self.run_for_all_coins('number_of_bursts')
    ...
    def distribution_of_time_between_bursts(self):
        ...
        '''
        return self.run_for_all_coins('time_between_bursts')
    ...
    def distribution_of_average_burst_size(self):
        ...
        '''
        return self.run_for_all_coins('average_size_of_each_burst')
    ...
    def distribution_of_average_number_of_users_per_burst(self):
        ...
        '''
        return self.run_for_all_coins('average_number_of_users_per_burst')
    ...
    def distribution_of_new_users_per_burst(self):
        ...
        '''
        return self.run_for_all_coins('new_users_per_burst')
    ...
    def distribution_of_burst_lifetime(self):
        ...
        '''
        return self.run_for_all_coins('lifetime_of_each_burst')
    ...
    def distribution_of_burst_platform_proportion(self):
        ...
        '''
        return self.run_for_all_coins('average_proportion_of_top_platform_per_burst')
