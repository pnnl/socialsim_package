...
class RecurrenceMeasurements(MeasurementsBaseClass):
    ...
    def get_time_between_bursts_distribution(self):
        if self.time_between_bursts_distribution is None:
            self.time_between_bursts_distribution = [(start_j - end_i).total_seconds() / (60*60*24) for (_, end_i), (start_j, _) in zip(self.burst_intervals[:-1], self.burst_intervals[1:])]
        return self.time_between_bursts_distribution
    ...
    def time_between_bursts(self):
        ...
        '''
        return np.mean(self.get_time_between_bursts_distribution())
