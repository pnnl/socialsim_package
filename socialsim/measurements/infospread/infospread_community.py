...
class InfospreadCommunity:
    ...
    def loadMetaData(self):
        """
        NOTE: This function does not load any data. Can we rename it to
            something that makes more sense?

        Description: Method splits the user meta data into location and
            creation date data frames.
    ...
    def burstsInCommunityEvents(self, eventTypes=None,
    ...
    def getCommunityPalma(self,communities=True, eventTypes=None,
        community_field="subreddit", content_field="root"):
        """
        Wrapper function calculate the Palma coefficient for the data frame.
        Question #6
        Input:
            :communities: (bool) Boolean to calculate the measurement seperately
                for each community (True) or for the full data set (False)
            :eventTypes: (list) A list of event types to include in the
                calculation.
        Output:
            :measurement: A dictionary of Palma coefficients for each community
        """
        if len(self.communityDF) > 0:
